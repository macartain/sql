...
select vd.name, vif.filetype_name, vif.small_read_megabytes SRMEG, vif.small_write_megabytes SWMEG, vif.small_read_servicetime SRSVC, vif.small_write_servicetime SWSVC,
            vif.large_read_megabytes LRMEG, vif.large_write_megabytes LWMEG, vif.large_read_servicetime LRSVC, vif.large_write_servicetime LWSVC
from v$iostat_file vif
    left join v$datafile vd on vd.file#=vif.file_no;

select vd.name, vf.* 
from v$filestat vf
    join v$datafile vd on vd.file#=vf.file#;
    
-- --------------------------------------------------------------------
-- Koopman - IOPS history 
-- http://www.databasejournal.com/features/oracle/article.php/3726391/Measuring-Disk-IO.htm
-- --------------------------------------------------------------------
...
SELECT to_char(end_time,'DDMONYYYY-HH24:MI'),
