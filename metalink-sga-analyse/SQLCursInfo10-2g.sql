...
DECLARE
   ...
   CURSOR code is select substr(sql_text,1,500),
      ...
      SEC_DEPTH_MISMATCH, EXPLAIN_PLAN_CURSOR, BUFFERED_DML_MISMATCH,
      ...
      OVERLAP_TIME_MISMATCH, SQL_REDIRECT_MISMATCH, MV_QUERY_GEN_MISMATCH,
      ...
      CURSOR_PARTS_MISMATCH,STB_OBJECT_MISMATCH,ROW_SHIP_MISMATCH,
      ...
      FLASHBACK_TABLE_MISMATCH,LITREP_COMP_MISMATCH
...
begin
     blankline:=chr(13);
     i:=0; k:=0;
     UC:=0; STM:=0; OPM:=0; OUM:=0; SRM1:=0; LM1:=0;
     SDM:=0; EPC:=0; BDM:=0; PEM:=0; IDM:=0; SQM:=0; 
     TM1:=0; ACM:=0; BM:=0; DM:=0; LM2:=0; TM2:=0; RLSM:=0; 
     IP:=0; IPR:=0;
     LSM:=0; ILM:=0; OTM:=0; SRM2:=0; MQGM:=0; UBPM:=0; 
     TDM:=0; NTM:=0; FC:=0; AT1:=0; IC:=0; TLRC:=0; 
     DLL:=0; LSA:=0; DCD:=0; BUD:=0; PCSD:=0; CPM:=0; 
     SOM:=0; RSM:=0; PSM:=0; TLDM:=0; MPM:=0; RIR:=0;
     BPPM:=0; MRM:=0; RIM:=0; OMM:=0; PM:=0; MSM:=0; 
     FTM:=0; LCM:=0; RTM:=0;
     open code;
     loop
        fetch code into 
        ...
        exit when code%NOTFOUND;
            ...
            IF 
               ROW_SHIP_MISMATCH='Y' 
            THEN   
               dbms_output.put_line('Mismatch due to Row Shipping');
               j:=j+1;
               RSM:=RSM+1;
            END IF;
            IF 
            ...
            IF j=0
            THEN
               dbms_output.put_line('WARNING <<<<< No Reasons Indicated');
               k:=k+1;
            ELSE
               dbms_output.put_line('Summary:  '||j||' Reasons Listed');
            END IF;
        end loop;
        dbms_output.put_line(blankline);
        dbms_output.put_line(blankline);
        ...
        IF SRM2 > 0
        THEN   
            dbms_output.put_line('SQL_REDIRECT_MISMATCH   '||SRM2);
        END IF;
        IF MQGM > 0
        THEN   
        ...
        IF RSM > 0
        THEN   
            dbms_output.put_line('ROW_SHIP_MISMATCH   '||RSM);
        END IF;
        IF CPM > 0
        THEN   
            dbms_output.put_line('CURSOR_PARTS_MISMATCH   '||CPM);
        ...
        IF MPM > 0
        THEN   
            dbms_output.put_line('MV_REWRITE_MISMATCH   '||MPM);
        END IF;
        IF RIM > 0
        THEN   
