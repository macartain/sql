...
DEFINE  owner_table = &upper_owner_tab
COLUMN spoolname FORMAT A50 NEW_VALUE spool_name NOPRINT

SELECT    'tab_idx_info_'||UPPER('&&owner_table')||'_'||TO_CHAR(SYSDATE, 'DDMONYYYYhh24miss')||'.log' spoolname
FROM       dual
/
...
COLUMN     tab_initrans			FORMAT 9999             HEADING 'Ini_t'
...
SELECT     TO_NUMBER(t.degree)                             tab_degree,
	   ...
	   t.ini_trans					   tab_initrans,
           t.num_rows                                      tab_num_rows,
           t.blocks                                        tab_alloc_blocks,
           (t.blocks - t.empty_blocks)                     tab_hwm_blocks,
           TO_CHAR(t.last_analyzed, 'DDMONYYYY hh24:mi')  tab_last_analyzed_time,
...
SELECT     i.index_name                                    ind_name,
           i.status                                        ind_status,
           i.ini_TRANS                                        initrans,
           DECODE(i.uniqueness,'UNIQUE','Y',NULL)          uniq,
           i.blevel                                        ind_blevel,
           i.leaf_blocks                                   ind_leafblocks,
           i.num_rows                                      ind_numrows,
           i.distinct_keys                                 ind_distinctkeys,
           i.clustering_factor                             ind_clufac,
           TO_CHAR(i.last_analyzed, 'DDMONYYYY hh24:mi')  last_analyzed_time
FROM       dba_indexes i
