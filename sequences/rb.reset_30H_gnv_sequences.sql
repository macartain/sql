...
REM
REM  Version: Schema 3.0H (for Infinys IRB 3.0.2-3.0.5)
REM
...
--     RATERDATASEQ
-- Not in doc, added below except CATALOGUEDISCOUNTSEQ:
--     ATTRSETATTRSEQ
--     BILLRUNSEQ
--     CATALOGUEDISCOUNTSEQ  (*** also not in schema ***)
--     CURRENTDELETESEQ
--     KEYMIGRATIONSEQ
--     PREDICATEIDSEQ
--     SORTMERGESEQ
...
--
-- Reset sequence ATTRSETATTRSEQ
-- 
prompt drop sequence ATTRSETATTRSEQ;;
select 'create sequence ATTRSETATTRSEQ increment by 1 start with '||
    (1+NVL(max(ATTR_SET_ATTR_SEQ),10000))
    || ' nomaxvalue nocache;' from ATTRIBUTESETATTRIBUTE;
--
...
--
-- Reset sequence BILLRUNIDSEQ
--
prompt drop sequence BILLRUNIDSEQ;;
select 'create sequence BILLRUNIDSEQ increment by 1 start with '||
    (1+NVL(max(BILL_RUN_ID),0))
    || ' nomaxvalue nocache;' from BILLRUN;
--
...
--
-- Reset sequence CATALOGUEDISCOUNTSEQ
-- 
prompt drop sequence CATALOGUEDISCOUNTSEQ;;
select 'create sequence CATALOGUEDISCOUNTSEQ increment by 1 start with '||
    (1+NVL(max(substr(CURRENT_DISCOUNT_SEQ)),0))
    || ' nomaxvalue nocache;' from CATALOGUECHANGE;
--
...
select 'create sequence CCMANDATESEQ increment by 1 start with '||
    (1+NVL(max(substr(mandate_ref,2,9)),100))
...
--
-- Reset sequence CURRENTDELETESEQ
-- 
prompt drop sequence CURRENTDELETESEQ;;
select 'create sequence CURRENTDELETESEQ increment by 1 start with '||
        (1+NVL(max(maxid),0))
    || ' nomaxvalue nocache;' from      
  (select max(current_delete_seq) as maxid from DELETEDINITIALINDEX1
    union 
   select max(current_delete_seq) as maxid from DELETEDINITIALINDEX2
    union 
   select max(current_delete_seq) as maxid from DELETEDPERIODICINDEX);
--
...
select 'create sequence DDMANDATESEQ increment by 1 start with '||
    (1+NVL(max(substr(mandate_ref,2,9)),100000))
    || ' nomaxvalue nocache;' from PRMANDATE
  where payment_method_id in (select payment_method_id from paymentmethod
...
--
-- Reset sequence KEYMIGRATIONSEQ
-- 
prompt drop sequence KEYMIGRATIONSEQ;;
select 'create sequence KEYMIGRATIONSEQ increment by 1 start with '||
    (1+NVL(max(KEY_MIGRATE_SEQ),0))
    || ' nomaxvalue nocache;' from KEYMIGRATION;
--
...
--
-- Reset sequence PREDICATEIDSEQ
-- 
prompt drop sequence PREDICATEIDSEQ;;
select 'create sequence PREDICATEIDSEQ increment by 1 start with '||
    (1+NVL(max(PREDICATE_ID),0))
    || ' nomaxvalue nocache;' from PREDICATE;
--
...
-- 
--  Reset sequence SORTMERGESEQ
--
prompt drop sequence SORTMERGESEQ;;
select 'create sequence SORTMERGESEQ increment by 1 start with '||
    (1+NVL(max(SORT_MERGE_SEQ),0))
    || ' nomaxvalue nocache;' from SORTMERGEDETAILS; 
--
...
--
-- Reset sequence UNLOADREQUESTIDSEQ
-- 
prompt drop sequence UNLOADREQUESTIDSEQ;;
select 'create sequence UNLOADREQUESTIDSEQ increment by 1 start with '||
    (1+NVL(max(UNLOAD_REQUEST_ID),0))
    || ' nomaxvalue nocache;' from UNLOADREQUEST;   
--
...
--
-- Reset sequence USTMAXTAXSEQ
-- 
-- prompt drop sequence USTMAXTAXSEQ;;
-- select 'create sequence USTMAXTAXSEQ increment by 1 start with '||
--  (1+NVL(max(???),0))
--  || ' nomaxvalue nocache;' from ???;      
-- -----------------------------------------------------------------
spool off
